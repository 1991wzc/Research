...
class Tf_idf:
    ...
    def calculateTf(self):
        ...
        for m in xrange(self.M):
            ...
            self.tf_idf[m]=[0]*N
            for n in xrange(N):
                    self.tf[m][n] = self.document[m].count(self.document[m][n])
...
def tf_idf(lan):
    ...
    parentpath=os.path.abspath(os.path.join(os.path.dirname("__file__"),os.path.pardir))
        ...
        print lan    
        TFIDF = Tf_idf()
        TFIDF.readNewWordmap('new_wordmap_'+lan+'.txt')
        TFIDF.readTweet(parentpath + '\\tweets\\'+lan+'_tweet_final.txt')
        TFIDF.calculateTf()
        TFIDF.calculateDf()
        ...
        TFIDF.creat_tf_idf_file('tf_idf_'+lan+'.txt')
